...
const Profile = () => {
    ...
    const handleSave = () => {
        setIsEditable(false);
        setEditedContent(null);
        const idCarry = { MentorId: user.MentorId };
        setUser(null);
        setReload(!reload);
        handleUpdateUser({ ...editedContent, ...idCarry }, "mentor");
    };
            ...
            if (!key.endsWith("_id")) {
                ...
                if (value) {
                    fetch(
                        ...
                        .then((rawdata) => {
                            const [data] = rawdata;
                            userData[keyName] = data.FirstName;
                        })
                        .catch((error) => {
                            throw new Error(error);
                        });
